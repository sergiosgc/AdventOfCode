...
pub fn main() !void {
    ...
    while (pairs.next()) |pair| {
        ...
        const left = pair[0].add(v.invert());
        const right = pair[1].add(v);
        if (left.x >= 0 and left.x <= input.max.x and left.y >= 0 and left.y <= input.max.y) try antinodes.put(left, {});
        if (right.x >= 0 and right.x <= input.max.x and right.y >= 0 and right.y <= input.max.y) try antinodes.put(right, {});
    }
    std.debug.print("{any}\n", .{antinodes.count()});
}
