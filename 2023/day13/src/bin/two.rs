...
pub fn is_range_symmetric(line: &HashSet<Coord>, x: i64, max_x: i64) -> bool {
    if (max_x - x + 1) % 2 != 0 {
        ...
        if to_test.len() % 2 != 0 {
            ...
            .reduce(|a,b| (a.0 && a.1 == b.1 && a.1 == x + max_x, a.1))
            .unwrap_or((true, 0)).0
        }

    }
}
pub fn smudged_horizontal_simmetry(puzzle: &mut Vec<HashSet<Coord>>) -> Option<f64> {
    let max = puzzle
    .iter()
    .flat_map(|line| line.iter().map(|c| c.clone()).collect::<Vec<Coord>>())
    .reduce(|a,b| Coord::bounding_max(&a, &b))
    .unwrap();
    let non_fudged_simmetry = horizontal_simmetry(puzzle, None);
    (0_i64..max.x+1)
    .cartesian_product(0_i64..max.y+1)
    .flat_map(|(x,y)| {
        if puzzle[y as usize].contains(& Coord { x, y }) {
            puzzle[y as usize].remove(& Coord { x, y} );
            let result = horizontal_simmetry(&puzzle, non_fudged_simmetry);
            puzzle[y as usize].insert(Coord { x, y });
            result
        } else {
            puzzle[y as usize].insert(Coord { x, y });
            let result = horizontal_simmetry(&puzzle, non_fudged_simmetry);
...
pub fn horizontal_simmetry(puzzle: &Vec<HashSet<Coord>>, avoid: Option<f64>) -> Option<f64> {
    let max_x = puzzle
    .iter()
    ...
    match
        ...
        .map(|line| (1..max_x)
            ...
            .filter(|(x1, x2)| avoid.is_none() || avoid.unwrap() != (x1 + x2) as f64 / 2_f64)
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("{:#?}", input.clone().into_iter().map(|mut puzzle| smudged_horizontal_simmetry(&mut puzzle)).map(|v| match v { Some(v) => v.ceil() as i64, None => 0 } ).zip(
                        input.into_iter().map(|puzzle| rotate(&puzzle)).map(|mut puzzle| smudged_horizontal_simmetry(&mut puzzle)).map(|v| match v { Some(v) => 100 * v.ceil() as i64, None => 0 } )
                    ).map(|(a,b)| a+b ).sum::<i64>());
    Ok(())
}