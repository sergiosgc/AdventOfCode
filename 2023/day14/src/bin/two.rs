...
impl Platform {
    ...
    pub fn rotate(&mut self) {
        ...
        .map(|(coord, rock)| (Coord { x: -coord.y + self.size.y, y: coord.x}, rock.clone()))
        .collect();
        self.size = Coord { x: self.size.y, y: self.size.x };
    }
    ...
    pub fn cycle_n(&mut self, cycles: i64) {
        let mut stop_cycle = cycles;
        let mut counter = 0;
        let mut visited = HashMap::<Platform, i64>::new();
        visited.insert(self.clone(), 0);
        loop {
            counter += 1;
            self.cycle();
            if let Some(cycle_start) = visited.get(self) {
                stop_cycle = (cycles - cycle_start) % (counter - cycle_start) + cycle_start + counter - cycle_start;
            }
            if counter == stop_cycle {
                return;
            }
            visited.insert(self.clone(), counter);
        }
