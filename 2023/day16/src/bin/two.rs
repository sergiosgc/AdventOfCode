...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("{:#?}", (0..input.size.x+1).flat_map(|x| vec![(Coord { x: x, y: 0 }, Coord { x: 0, y: 1 }), (Coord { x: x, y: input.size.y }, Coord { x: 0, y: -1 }) ]).chain(
        (0..input.size.y+1).flat_map(|y| vec![(Coord { x: 0, y: y }, Coord { x: 1, y: 0 }), (Coord { x: input.size.x, y: y }, Coord { x: -1, y: 0 }) ])
    ).map(|(pos, dir)| energized(pos, dir, &input, &mut HashSet::new()).len())
    .max()
    );
    Ok(())
}
