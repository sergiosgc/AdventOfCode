...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    .map(|(damaged, groups)| (
        vec![damaged.clone(), damaged.clone(), damaged.clone(), damaged.clone(), damaged.clone()].join("?"),
        vec![groups.clone(), groups.clone(), groups.clone(), groups.clone(), groups.clone()].into_iter().flatten().collect::<Vec<i64>>()
    ))
