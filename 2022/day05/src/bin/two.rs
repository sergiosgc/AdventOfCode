...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("{:#?}", moves
        ...
        .fold(stacks, |mut stack, (count, from, to)| {
            let stack_from = stack[from].clone();
            let (left, right) = stack_from.split_at(stack_from.len() - count as usize);
            right.into_iter().for_each( |r| stack[to].push(r.clone()) );
            stack[from] = left.to_vec();
            stack
        })
        .into_iter()
