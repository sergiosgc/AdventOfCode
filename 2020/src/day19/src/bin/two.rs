...
impl Rule {
    ...
    fn is_match(&self, s: String, rules: &Rules, depth: i32) -> Option<String> {
        if depth > 1000 { return None; }
        match (self.either.as_ref(), self.sequence.as_ref(), self.char) {
            (Some(either), None, None) => {
                let result = either.0.is_match(s.clone(), rules, depth+1);
                if result.is_some() {
                    result
                } else {
                    either.1.is_match(s, rules, depth+1)
                }
            },
            (None, Some(sequence), None) => {
                let mut result = s.clone();
                for index in sequence.into_iter() {
                    match rules.0[index].is_match(result, rules, depth+1) {
                        None => return None,
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("{:#?}", input.into_iter()
        ...
        .filter(|l| l.is_some())
        .map(|l| l.unwrap())
        .map(|l| rules.0[&0].is_match(l.clone(), &rules, 0) )
        .filter(|l| l.is_some())
        .map(|l| l.unwrap())
        .filter(|l| l.len() == 0)
        .collect::<Vec<String>>()
